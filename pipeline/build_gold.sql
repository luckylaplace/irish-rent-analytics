...
FROM
    ...
    JOIN gold.dim_location l ON s.location = l.location
    AND s.county = l.county
    AND s.area = l.area
    -- 2. Mülk özellikleri eşleştirmesi
    JOIN gold.dim_property p ON s.property_type = p.property_type
    AND s.bedrooms = p.bedrooms
    -- 3. Zaman eşleştirmesi
    JOIN gold.dim_time t ON s.rent_year = t.rent_year
    AND s.half = t.half
ON CONFLICT DO NOTHING;