...
TRUNCATE TABLE silver.rent_clean;
...
SELECT source_file,
    ...
    CAST(
        NULLIF(NULLIF(rent_euro, 'NaN'), '') AS NUMERIC
    ) AS rent_euro,

-- Yıl ve Yarım Yıl (Half) bilgisini tam sayıya (INTEGER) çeviriyoruz
CAST(
    CAST(
        NULLIF(NULLIF(year, 'NaN'), '') AS NUMERIC
    ) AS INTEGER
) AS rent_year,
CAST(
    CAST(
        NULLIF(NULLIF(half, 'NaN'), '') AS NUMERIC
    ) AS INTEGER
) AS half,
...
(
    LOWER(is_county_aggregate) = 'true'
) AS is_county_aggregate
...
-- 0 veya negatif kiraları işleme almıyoruz